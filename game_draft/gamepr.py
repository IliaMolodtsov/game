...
pygame.init()
screen = pygame.display.set_mode((768, 432))  # 669 1099
SCREEN_HEIGHT = 432
SCREEN_WIDTH = 768
...
bg_images = []
for k in range(1, 6):
    bg_image = pygame.image.load(f'images\parallax\plx-{k}.png').convert_alpha()
...
def draw_bg():
    for x in range(100):
        speed_bg = 1
        for i in bg_images:
            bg_xy = (x * bg_width) - scroll * speed_bg
            screen.blit(i, (bg_xy, 0))
            speed_bg += 0.2
    for b in range(100):
        speed_bg = 1
        for i in bg_images:
...
def menu():
    ...
    while show:  # пока показываем меню
        ...
        start_button.draw(290, 120, 'Play', None, 35)  # указываем координаты кнопки, на посл месте размер шрифта
        quit_button.draw(290, 270, "Quit", quit, 35)
...
player_y = 365  # координаты игрока по y. Чем больше координаты, тем НИЖЕ ИГРОК. Чем меньше координаты, тем игрок ВЫШЕ.
...
enemy_land_y = 315  # координаты наземного врага по y
...
mushroom_x = 500  # координаты наземного врага по x
mushroom_y = 220  # координаты наземного врага по y

mb_x = 680  # координаты пули гриба по оси X
mb_y = 360  # координаты пули гриба по оси Y
mb_speed = 9  # скорость полета пули гриба
mb_sprite = img('bullet/bullet_mushroom.png')
mb_hit = False
# указывает, попала ли пуля в игрока, если эта переменная имеет значение True то пуля отображаться не будет
...
runner_x = 350  # координаты бегуна по x
runner_y = 327  # координаты бегуна по y
...
enemy_sky_x = 650  # координаты летающего врага по x
enemy_sky_y = 215  # координаты летающего врага по y
...
while running:
    ...
    # for k in range(0, 5):
    #  screen.blit(bg_images[k], (bg_x, 0))  # продолжаем бэкграунд вправо и влево
    # screen.blit(bg_images[k], (bg_x + 768, 0))
    # screen.blit(bg_images[k], (bg_x - 768, 0))
    ...
    if enemy_land_x not in range(0, 768 - 104): enemy_land_dir = -enemy_land_dir
    ...
    if runner_x not in range(0, 768 - 104): runner_dir = -runner_dir

    # летающий враг летит от одного края экрана к другому и обратно
    enemy_sky_x += enemy_sky_speed * enemy_sky_dir
    if enemy_sky_x not in range(0, 768 - 98): enemy_sky_dir = -enemy_sky_dir
    ...
    # пуля гриба
    # Если таймер достиг нужного числа кадров, выпускаем пулю
    if mb_x < 0:
        mb_x = 600  # возвращаем координату X пули к начальному значению
        mb_hit = False
    # Если нет, двигаем пулю в нужном направлении и увеличиваем счетчик таймера
    else:
        mb_x -= mb_speed
    # Если пуля в игрока не попала, запускаем новую
    if not mb_hit: screen.blit(mb_sprite, (mb_x, mb_y))
    # бегун
    ...
    if mushroom_delay_frame == mushroom_delay:
        # проверяем количество прошедших кадров, чтобы понять, пора ли сменять кадр
        mushroom_delay_frame = 0
        if mushroom_anim_count == 1:
            # Обнуляем индекс, когда доходим до последнего элемента списка (чтобы не выйти за него)
            mushroom_anim_count = 0
        else:
            mushroom_anim_count += 1
            # каждый раз, заходя в цикл, будем увеличивать индекс, выводя поочередно все элементы
    else:
    ...
    # ВНЕДРЕНИЕ СОБИРАЕМОГО СЕРДЕЧКА

    heart = img('heart.png')

    if not heart_collect: screen.blit(heart, (700, 380))
    ...
    if keys[pygame.K_LEFT] or keys[pygame.K_a]:
        ...
        # при этом выводя все элементы списка по индексу указанной в кв скобках переменной.
        # Это для эффекта движения влево
    ...
    if (keys[pygame.K_LEFT] or keys[pygame.K_a]) and player_x > 85:
        ...
        if player_x <= frame_left:  # настраиваем движение рамки игрока и движение бэкграунда при нажатии "влево"
            ...
            # if bg_x == 1100:
            #     bg_x = 0
    elif (keys[pygame.K_RIGHT] or keys[pygame.K_d]) and player_x < 700:
        ...
        if player_x >= frame_right:  # настраиваем движение рамки игрока и движение бэкграунда при нажатии "вправо"
            ...
            # if bg_x == -1100:
            # bg_x = 0
    elif (keys[pygame.K_LEFT] or keys[pygame.K_a]) and player_x <= 85:
        # Если эта кнопка равнёа стрелке влево или букве A, да и коорд игр больше 50 (невидимая граница слева), то:
    ...
    mb_hitbox = Rect(mb_x, mb_y, 18, 15)
    ...
    collision = collision or Rect.colliderect(player_hitbox, mb_hitbox)
    ...
    # скрываем пулю, если она достигла игрока
    if Rect.colliderect(player_hitbox, mb_hitbox): mb_hit = True

    # ВЫЯВЛЕНИЕ КОЛЛИЗИЙ СЕРДЕЧЕК И ИГРОКА

    # Если сердце уже собрано, ничего ниже выполняться не будет
    if not heart_collect:

        # Создаем хитбокс сердечка
        heart_hitbox = Rect(700, 380, 16, 16)

        # Выявляем коллизию хитбоксов сердечка и игрока
        heart_collect = Rect.colliderect(player_hitbox, heart_hitbox)

        # Пополняем жизни при наличии коллизии
        if heart_collect: player_lives += 1
    ...
    # РИСОВАНИЕ СЕРДЕЧЕК ЖИЗНИ

    # Рисуем сердечки в левом верхнем углу в количестве числа жизней у игрока
    # Спрайти используется тот же что и для собираемых сердечек
    for i in range(player_lives):
        screen.blit(heart, (20 * (i + 1), 20))
