...
mb_x = 817  # координаты пули гриба по оси X
mb_y = 550  # координаты пули гриба по оси Y
mb_speed = 9  # скорость полета пули гриба
mb_direction = ''  # направление пули
mb_delay = 100  # количество кадров в промежутке между выпусками пуль
mb_timer = 100  # количество кадров прошедших с предыдущего выпуска пули
mb_sprite = img('bullet/bullet_mushroom.png')
mb_hit = False  # указывает попала ли пуля в игрока, если эта переменная имеет значение True то пуля отображаться не будет
...
while running:
    ...
    if mb_timer == mb_delay:
        # Возвращаем пулю на начальную позицию
        mb_x = 817
        mb_y = 550
        # Определяем направление пули
        if player_x < mushroom_x:
            mb_direction = 'l'
        elif player_x > mushroom_x + 234:
            mb_direction = 'r'
        elif player_y < 550:
            mb_direction = 'u'
        else:
    ...
    if not heart_collect: screen.blit(heart, (1000, 555))
    ...
    if not heart_collect:
        ...
        heart_hitbox = Rect(1000, 555, 16, 16)
